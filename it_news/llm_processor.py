...
from prompts import (
    TECH_NEWS_PROMPT, AI_NEWS_PROMPT, 
    PROGRAMMING_NEWS_PROMPT, SECURITY_NEWS_PROMPT
)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("it_llm.log"),
...
class LLMProcessor:
    ...
    def get_prompt_for_category(self, category):
        ...
        return category_prompts.get(category, TECH_NEWS_PROMPT)
        
    def format_article_with_llm(self, article):
        """Process an article with the LLM API to create a formatted post"""
        prompt_template = self.get_prompt_for_category(article.get("category", "tech"))
