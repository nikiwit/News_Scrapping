...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("business_news/logs/business_scraper.log"),
...
class NewsScraperLegal:
    ...
    def save_news(self, articles):
        """Save extracted news to files with timestamped filenames"""
        if not articles:
        ...
        filename = f"business_news_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    if action == "scrape":
        print("Scraping news...")
        filepath = scraper.scrape_all_sources()
        if filepath:
        ...
        if not files:
            ...
            file_index = input("Enter file number to view contents, or 0 to exit: ")
            try:
                file_index = int(file_index)
                if 1 <= file_index <= len(files):
                    with open(files[file_index-1], 'r', encoding='utf-8') as f:
                        data = json.load(f)
                        print(f"\nFile contains {len(data)} articles.\n")
                        for i, article in enumerate(data):
                            print(f"Article {i+1}: {article['title']} ({article['source']})")
